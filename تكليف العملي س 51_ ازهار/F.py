...
if anova_results['PR(>F)'][0] < alpha:
    ...
    # حساب الارتباط بين الأعمدة العددية
    corr = df[['Sales', 'Customer_Age']].corr()

    plt.figure(figsize=(8, 6))
    sns.heatmap(corr, annot=True, cmap='coolwarm', center=0)
    plt.title('خريطة الحرارة لتحليل الارتباط')
    plt.savefig('correlation_heatmap.png')
    plt.show()

    plt.figure(figsize=(8, 8))
    region_sales.plot(kind='pie', autopct='%1.1f%%', startangle=90, colors=['#ff9999', '#66b3ff', '#99ff99', '#ffcc99'])
    plt.title('نسبة المبيعات في كل منطقة')
    plt.ylabel('')
    plt.savefig('sales_pie_chart.png')
    plt.show()

    plt.figure(figsize=(12, 6))
    product_age = df.groupby('Product')['Customer_Age'].mean().sort_values(ascending=False).head(10)
    product_age.plot(kind='bar', color='orange')
    plt.title('متوسط أعمار العملاء لأهم 10 منتجات')
    plt.xlabel('المنتج')
    plt.ylabel('متوسط عمر العميل')
    plt.xticks(rotation=45)
    plt.grid(True, alpha=0.3)
    plt.savefig('average_age_by_product.png')
    plt.show()

    top_days = daily_sales.nlargest(5)

    plt.figure(figsize=(10, 6))
    top_days.plot(kind='bar', color='green')
    plt.title('أعلى 5 أيام من حيث المبيعات')
    plt.xlabel('التاريخ')
    plt.ylabel('إجمالي المبيعات')
    plt.xticks(rotation=45)
    plt.grid(True, alpha=0.3)
